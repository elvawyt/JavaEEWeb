...
<%String rank=(String)request.getParameter("rank"); 
  ...
  String sourcecontent=(String)request.getParameter("sourcecontent");
  String permission=(String)request.getParameter("permission");
  String pathh=(String)request.getParameter("path");
...
　　                  
	...
	<body>
		...
		<div class="reg">
			...
			<div class="row" ><center><img class="media-object" src="img/pic6.jpg" style="max-width:100%;" alt="..." ></center></lable>
				</div>
				<input  value="<%=rank%>" id="rank">
				<input  value="<%=permission%>" id="permission">
				<input style="display:none" value="<%=pathh%>" id="pathh" name="pathh">
				<a href="#" class="submit" onclick="javascript:jug()">点我下载喔！</a>
			</form>
		</div>
		<script type="text/javascript">
				function jug(){
					...
					if(permission==1){ 
						if(rank=="vip"){
							th.action="<%=path%>/DLServlet";  
							th.submit();	
						}
						else{
							alert("抱歉，本资源仅限VIP用户下载，可升级为VIP再来下载喔！");
							}
					}
					else
						{
