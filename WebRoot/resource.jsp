    ...
    <div class="thumbnail">
     <%System.out.println(reso.getSourcetype()); %>
      ...
      <div class="caption">
        ...
        <p>...</p>
        <p><a href="try.jsp?sourcename=<%=reso.getSourcename() %>&&sourcecontent=<%=reso.getSourcecontent() %>&&permission=<%=reso.getPermission() %>&&path=<%=reso.getPath() %>&&rank=<%=User.getRank() %>" class="btn btn-primary" role="button">Read more</a></p>
      </div>
    </div>
  </div>
  <% }} %>
